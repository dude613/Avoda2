  ...
  <div class="flex items-center justify-center min-h-screen bg-gray-100">
    <div class="bg-white p-8 rounded-2xl shadow-lg w-full max-w-md">
      ...
      <form @submit.prevent="loginWithPassword">
        ...
        <input v-model="email" type="email" placeholder="you@example.com" class="w-full px-3 py-2 border rounded-lg mb-4 text-black">
        
        <label class="block mb-2 text-sm font-medium text-black">Password</label>
        <input v-model="password" type="password" class="w-full px-3 py-2 border rounded-lg mb-6 text-black">
        
        <button type="submit" class="w-full bg-black text-white py-2 rounded-lg hover:opacity-90 mb-4">Login</button>
      </form>

      <div class="text-center text-sm mb-2 text-gray-500">
        Don't have an account? 
        <router-link to="/auth/register" class="text-black font-semibold hover:underline">Sign up</router-link>
      </div>

      <div class="text-center text-sm mt-6">
        <div class="text-center text-sm mt-6">
  <router-link to="/auth/forgot-password" class="text-black font-semibold hover:underline">
    Forgot password?
  </router-link>
</div>
      </div>
